...
const visitData = [];
const revenueMonthByMonth = [];
const PostMonthByMonth = [];
...
for (let i = 0; i < 12; i += 1) {
  revenueMonthByMonth.push({
    x: 'Tháng ' + i,
    y: Math.floor(Math.random() * 20000000)
  });
}
for (let i = 0; i < 12; i += 1) {
  PostMonthByMonth.push({
    x: 'Tháng ' + i,
        ...
        <Row>
          <Col span={6} className="card-show-data">
            <ChartCard
              ...
              total={() => (
                <span
                  dangerouslySetInnerHTML={{ __html: numeral(230970000).format('0,0') }}
                />
              )}
              footer={<Field label="Doanh thu tháng (12)" value={numeral(60000000).format('0,0')}/>}
              contentHeight={60}
            >
              <MiniArea line height={45} data={revenueMonthByMonth}/>
            </ChartCard>
          </Col>
          <Col span={6} className="card-show-data">
            <ChartCard
              title="Tổng số bài viết năm 2019"
              action={
                <Tooltip title="Tổng số bài viết trong năm 2019">
                  <Icon type="info-circle-o"/>
                </Tooltip>
              }
              total={numeral(960).format('0,0')}
              footer={<Field label="Bài viết tháng (12)" value={numeral(80).format('0,0')}/>}
              contentHeight={60}
            >
              <MiniBar height={60} data={PostMonthByMonth}/>
            </ChartCard>
          </Col>
          <Col span={6} className="card-show-data">
            <ChartCard
